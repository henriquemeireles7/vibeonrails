...
function formatExpiryTime(expiresAt: string | null, isExpired: boolean): string {
...
export function formatConnectionStatus(status: ConnectionStatus): string {
  ...
  if (status.lastRefreshedAt) {
    ...
    lines.push(`    ${chalk.dim('Last refresh:')} ${refreshDate.toLocaleString()}`);
  }
  ...
  connections
    ...
    .action(async (provider?: string) => {
      try {
        ...
        const { createFileTokenStore } = await import('@vibeonrails/core/security');
        ...
        if (provider) {
          ...
          if (statuses.length === 0) {
            ...
            console.log('  Run ' + chalk.cyan('vibe connect <provider>') + ' to add a connection.\n');
            return;
          }
