...
describe('Deploy Command', () => {
  ...
  describe('isDestructiveMigration', () => {
    ...
    it('should detect DROP COLUMN as destructive', () => {
      expect(isDestructiveMigration('ALTER TABLE users DROP COLUMN email;')).toBe(true);
    });

    it('should detect RENAME as destructive', () => {
      expect(isDestructiveMigration('ALTER TABLE users RENAME TO people;')).toBe(true);
    });
    ...
    it('should detect DELETE FROM as destructive', () => {
      expect(isDestructiveMigration('DELETE FROM users WHERE active = false;')).toBe(true);
    });

    it('should not flag additive operations', () => {
      expect(isDestructiveMigration('ALTER TABLE users ADD COLUMN bio TEXT;')).toBe(false);
      expect(isDestructiveMigration('CREATE TABLE posts (id SERIAL);')).toBe(false);
      expect(isDestructiveMigration('CREATE INDEX idx_users_email ON users (email);')).toBe(false);
    });
  ...
  describe('healthCheck', () => {
    it('should return healthy on 200 response', async () => {
      ...
      const result = await healthCheck('http://localhost:3000/health', 5, mockFetch);
    ...
    it('should retry on non-200 responses', async () => {
      ...
      const result = await healthCheck('http://localhost:3000/health', 30, mockFetch);
    ...
    it('should timeout and return unhealthy', async () => {
      ...
      const result = await healthCheck('http://localhost:3000/health', 1, mockFetch);
