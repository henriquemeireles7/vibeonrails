...
describe('Migration Create', () => {
  ...
  describe('classifyOperation', () => {
    it('should classify CREATE TABLE as additive', () => {
      const result = classifyOperation('CREATE TABLE users (id SERIAL PRIMARY KEY)');
    ...
    it('should classify CREATE INDEX as additive', () => {
      const result = classifyOperation('CREATE INDEX idx_users_email ON users (email)');
      expect(result.type).toBe('additive');
    });
  ...
  describe('parseOperations', () => {
    it('should parse multiple SQL statements', () => {
      const sql = 'CREATE TABLE users (id SERIAL); ALTER TABLE users ADD COLUMN email TEXT';
  ...
  describe('describeOperation', () => {
    it('should describe CREATE TABLE', () => {
      expect(describeOperation('CREATE TABLE users (id SERIAL)')).toBe('Create table "users"');
    });

    it('should describe ADD COLUMN', () => {
      expect(describeOperation('ALTER TABLE users ADD COLUMN bio TEXT')).toBe('Add column "bio" to "users"');
    });

    it('should describe DROP COLUMN', () => {
      expect(describeOperation('ALTER TABLE users DROP COLUMN bio')).toBe('Drop column "bio" from "users"');
    });

    it('should describe CREATE INDEX', () => {
      expect(describeOperation('CREATE INDEX idx_users_email ON users (email)')).toBe('Create index "idx_users_email"');
    });
    ...
    it('should describe DROP INDEX', () => {
      expect(describeOperation('DROP INDEX idx_users_email')).toBe('Drop index "idx_users_email"');
    });
  ...
  describe('reverseOperation', () => {
    it('should reverse CREATE TABLE to DROP TABLE', () => {
      expect(reverseOperation('CREATE TABLE users (id SERIAL)')).toBe('DROP TABLE IF EXISTS users;');
    });
  ...
  describe('formatMigrationFile', () => {
    it('should generate valid TypeScript migration file', () => {
      const migration: GeneratedMigration = {
        ...
        name: 'add-bio',
        up: [
          { sql: 'ALTER TABLE users ADD COLUMN bio TEXT;', type: 'additive', description: 'Add column "bio" to "users"' },
        ],
        down: [
          { sql: 'ALTER TABLE users DROP COLUMN IF EXISTS bio;', type: 'destructive', description: 'Drop column "bio" from "users"' },
        ],
    ...
    it('should mark destructive migrations with warning', () => {
      const migration: GeneratedMigration = {
        ...
        name: 'drop-legacy',
        up: [
          { sql: 'DROP TABLE legacy;', type: 'destructive', description: 'Drop table "legacy"' },
        ],
        down: [
          { sql: '-- TODO: Manual rollback needed for: Drop table "legacy"', type: 'additive', description: 'Manual rollback' },
        ],
        hasDestructive: true,
