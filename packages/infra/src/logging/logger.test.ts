...
describe('Logger', () => {
  ...
  it('should log debug messages', () => {
    const lines: string[] = [];
    const log = new Logger({ writer: (line) => lines.push(line), minLevel: 'debug' });
  ...
  it('should include module in JSON output', () => {
    process.env.NODE_ENV = 'production';
    const lines: string[] = [];
    const log = new Logger({ module: 'api', writer: (line) => lines.push(line) });
  ...
  it('should propagate requestId through child loggers', () => {
    process.env.NODE_ENV = 'production';
    const lines: string[] = [];
    const parent = new Logger({ module: 'api', writer: (line) => lines.push(line) });
  ...
  it('should respect minimum log level', () => {
    const lines: string[] = [];
    const log = new Logger({ writer: (line) => lines.push(line), minLevel: 'warn' });
  ...
  it('should include module name in dev format', () => {
    const lines: string[] = [];
    const log = new Logger({ module: 'queue', writer: (line) => lines.push(line) });
  ...
  it('should create logger via factory', () => {
    const lines: string[] = [];
    const log = createLogger({ module: 'test', writer: (line) => lines.push(line) });
