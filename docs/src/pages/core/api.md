...
|--------|------|---------|-------------|
| `router` | `AnyRouter` | *required* | The tRPC router to serve |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `max` | `number` | *required* | Maximum requests within the window |
...
```typescript
import { NotFoundError, ValidationError, ForbiddenError } from '@vibeonrails/core/shared';
