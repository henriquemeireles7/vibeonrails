...
export async function create(userId: string, input: CreateTodoInput): Promise<Todo> {
  const [todo] = await db
    .insert(todos)
    .values({
      ...input,
...
export async function toggleComplete(id: string, userId: string): Promise<Todo> {
...
describe('Todo Service', () => {
  ...
  describe('toggleComplete', () => {
    it('should toggle completed status', async () => {
      const todo = await todoService.create(TEST_USER_ID, { title: 'Toggle me' });
      ...
      const toggledBack = await todoService.toggleComplete(todo.id, TEST_USER_ID);
  ...
  describe('remove', () => {
    it('should delete a todo', async () => {
      const todo = await todoService.create(TEST_USER_ID, { title: 'Delete me' });
      await todoService.remove(todo.id, TEST_USER_ID);

      await expect(
        todoService.getById(todo.id, TEST_USER_ID),
